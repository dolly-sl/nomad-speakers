    ...
    $(cartObject.items).each(function () {
      ...
      */
      ...
      if ($(".cart-list")[0]) {
        ...
        cartLineItem += '<div class="cart-list__items__columns price" > ' + showPrice;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const searchBtn = document.getElementById("js__header-search-btn") || document.querySelector(".header .toggle-icon") || document.querySelector(".toggle-icon");
  const body = document.body;

  // 🧠 Check if page has hero section
  const hasHero = !!document.querySelector(".hero-banner, .inner-hero-section, .error-page, .collection-hero-section");
  ...
  document.addEventListener("keydown", e => {
    if (e.key === "Escape" && searchBtn?.dataset.open === "true") closeSearchBar();
  });
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.querySelectorAll(".has-sub-nav").forEach(item => {
    ...
    const showSubNav = () => {
      ...
      document.querySelectorAll(".header-search-section.visible, .js__search.active").forEach(el => el.classList.remove("visible", "active"));
    };
    const hideSubNav = () => {
  ...
  if (window.innerWidth > 980) {
    ...
    document.querySelectorAll(".has-big-nav").forEach(function (item) {
      ...
      item.addEventListener("click", function (event) {
        ...
        const searchSection = document.getElementById("js-header-search-section");
        const isSearchOpen = searchBtn?.dataset.open === "true" || searchSection?.classList.contains("visible");
        if (isSearchOpen) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const nav = document.getElementById("navbarNavDropdown");
  const header = document.getElementById("mainHeader");
  const hideEls = document.querySelectorAll(".js__mobile-menu-open-hide, .js__mobile-announcement-text");
  ...
  const toggleMenu = () => {
    ...
    navLink = true;
    hideEls.forEach(el => nav.classList.contains("active") ? el.classList.add("active") : el.classList.remove("active"));
    ...
    if (nav.classList.contains("active")) {
      ...
      document.documentElement.style.overflowY = "scroll";
      document.documentElement.classList.remove("scroll-stop");
      document.querySelectorAll("body .boost-pfs-search-suggestion-group").forEach(el => el.style.display = "none");
    }
    updateHeaderState();
  };
...
document.addEventListener("DOMContentLoaded", function () {
  const header = document.getElementById("mainHeader");
  const heroBanner = document.querySelector(".hero-banner, .inner-hero-section, .error-page, .collection-hero-section");
  const announcement = document.querySelector(".announcement-bar");
  const hamburger = document.getElementById("hamburger");
